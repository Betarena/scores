  ...
  if (dev) console.debug(`odds/${year_}/${new_month_}/${day_}/${fixtureId}`);
	...
	return get(child(ref(db_real), `odds/${year_}/${new_month_}/${day_}/${fixtureId}`)).then(
        ...
        if (dev) console.debug("Here! Testing!");
				...
				throw new Error(`Data from getTargetFixtureOdds() method for Firebase DB is incorrect => odds/${year_}/${new_month_}/${day_}/${fixtureId} does not exist! Date: ${fixture_data.date} | Day: ${day_} Day_In_UTC: ${new Date(fixture_data.date).getUTCDay()} | Game Start: ${fixture_data.game_start} Lang: ${fixture_data.lang}`);
        // return null;
			}
		}
	);
