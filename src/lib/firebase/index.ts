...
export async function getTargetFixtureOdds(
	...
	return get(
		...
		if (snapshot.exists()) {
			...
			const fixture_odds_keys = Object.keys(
				snapshot.val()
			);
			...
			for (const rankedOdd of Object.keys(
				sportbook_details
			)) {
				// ... iterate over the data of the fixture avaiable ODDS;
				for (const avaiableOdd of Object.keys(
					fixture_odds_keys
				)) {
					// ...
					const targetFixture =
						fixture_odds_keys[avaiableOdd];
					const fixtureOdd =
						fixture_odds[targetFixture];
					// ... check for a match of the odds names;
					if (
						fixture_odds_keys[avaiableOdd]
							.toString()
							.toLowerCase() ==
							sportbook_details[rankedOdd][
								'title'
							]
								.toString()
								.toLowerCase() &&
						fixtureOdd['markets']['1X2FT'] !=
							null &&
			...
			throw new Error(
				`Data from DB is incorrect: getTargetFixtureOdds() fixture_data ${fixture_data}`
			);
		}
	});
}
 ...
 * @param userGeoLocation
 */
export async function getTargetGeoSportBookDetails(
	lang: string,
	siteName?: string
): Promise<unknown> {
	// ... return the odds-site info & the odds values;
	if (siteName != undefined) {
			...
			if (snapshot.exists()) {
				...
				for (const rankedOdd in sportbook_details_) {
					...
					if (
						...
						sportbook_details_[rankedOdd]['title']
							.toLowerCase()
							.toString()
					) {
						map.set(
							'betting_site_info',
							sportbook_details_[rankedOdd]
						);
					}
				}
				...
				throw new Error(
					`Data from DB is incorrect: getTargetGeoSportBookDetails() lang: ${lang} sitename: ${siteName}`
				);
			}
		});
	} else {
		return get(
			...
			if (snapshot.exists()) {
				...
				throw new Error(
					`Data from DB is incorrect: getTargetGeoSportBookDetails() lang-only: ${getTargetGeoSportBookDetails}`
				);
			}
		});
	}
