...
function createLocalStore(key) {
	...
	return {
		...
		addToVotes: (fixture_Vote: unknown) => {
			...
			const existingData: fixtureVoteData =
				existing
					? JSON.parse(existing)
					: {
							fixtureVotes_Array: []
			...
			const newCartMap =
				existingData.fixtureVotes_Array.map(
					(p) => {
						// if fixture_id already exists,
						if (
							p.fixture_id ===
							fixture_Vote.fixture_id
						) {
			...
			if (changed) {
				...
				existingData.fixtureVotes_Array =
					newCartMap;
			} else {
				// otherwise, add the new item to the cart-array;
				existingData.fixtureVotes_Array.push(
					fixture_Vote
				);
			}
		...
		removeItem: (fixture_Vote) => {
			...
			existingData.fixtureVotes_Array.forEach(
				(element) => {
					...
					if (
						...
						existingData.fixtureVotes_Array.splice(
							i,
							1
						);
					}
...
export const fixtureVote = createLocalStore(
	'betarena-widget-featured-match'
);
