<script lang="ts">
  ...
  import Input from "./Input.svelte";
	...
	let yearPageStart = 0;
	$: prevLabel = viewMode === "day" ? "Previous month" : viewMode === "month" ? "Previous year" : "Previous years";
	$: nextLabel = viewMode === "day" ? "Next month" : viewMode === "month" ? "Next year" : "Next years";
	...
	function getYearPageStart(year: number) {
		...
		return Math.min(maxStart, Math.max(rangeStart, rangeStart + pageIndex * YEAR_PAGE_SIZE));
	}
		...
		{#if showActions}
			<div class="calendar-actions">
                <div class="date-input-wrapper">
                    ...
                    <Input size="sm"  readonly={true} placeholder={placeholder} value={displayValue}/>
                </div>
			...
			<div class="calendar-grid">
				{#each calendarCells as cell}
					<button
						...
						class:calendar-cell--selected={cell.isSelected || cell.date.getTime() === value?.getTime()}
...
<style>
	.calendar {
		...
		border: 1px solid var(--calendar-border, rgba(0, 0, 0, 0.08));
		...
		color: var(--calendar-text, #525252);
		font-family: var(
	...
	.calendar-content {
		padding: var(--calendar-padding-y, 20px) var(--calendar-padding-x, 24px);
		display: flex;
		flex-direction: column;
		gap: var(--calendar-section-gap, 12px);
	}
	...
	.calendar-nav {
		...
		color: var(--calendar-nav-color, #525252);
		cursor: pointer;
	}
	...
	.calendar-title-text {
		font-size: var(--calendar-title-size, 14px);
		font-weight: var(--calendar-title-weight, 600);
		color: var(--calendar-title-color, #525252);
	}

	.calendar-actions {
		display: flex;
		gap: var(--calendar-actions-gap, 12px);
	}
	...
	.calendar-weekdays {
		...
		text-align: center;
		font-size: var(--calendar-weekday-size, 14px);
		font-weight: var(--calendar-weekday-weight, 500);
		color: var(--calendar-weekday-color, #525252);
	}
	...
	.calendar-cell {
		...
		border: none;
		background: transparent;
		color: var(--calendar-day-color, #525252);
		font-size: var(--calendar-day-size, 14px);
		font-weight: var(--calendar-day-weight, 400);
		position: relative;
		display: flex;
		align-items: center;
		justify-content: center;
	...
	.calendar-cell--outside {
		color: var(--calendar-outside-color, #727171);
	}

	.calendar-cell--disabled {
		color: var(--calendar-disabled-color, #727171);
		cursor: not-allowed;
	}
	...
	.calendar-cell--today {
		background: var(--calendar-today-bg, #fbfbfb);
		font-weight: var(--calendar-today-weight, 500);
	}

	.calendar-cell--selected {
		background: var(--calendar-selected-bg, #f5620f);
	...
	.calendar-footer {
		border-top: 1px solid var(--calendar-footer-border, #ededed);
