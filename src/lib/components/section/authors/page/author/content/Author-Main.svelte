    ...
    if (paid && !accessGranted) {
      ...
      if (blurContentNode && target) {
        try {
          ...
          const blurHeight = blurContentNode.getBoundingClientRect().height
...
<div id={CNAME} data-betarena-zone-id="4" class={viewportType} class:paid={paid && !accessGranted}>
  ...
  {#key $userSettings.theme}
    <div id="content" data-betarena-zone-id="2,3" bind:this={contentContainer}>
      ...
      {/key}
      {#if paid && !accessGranted}
        <div class="blur-content" id="content-blur" bind:this={blurContentNode} />
      {/if}
    </div>
