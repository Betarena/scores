...
<script lang="ts">
  ...
  import { B_C_LIN_F_Q_D1 } from "@betarena/scores-lib/dist/graphql/query.lineups.js";
	...
	let home_team_formation_map = new Map<
		string,
		Fixture_Player[]
	>();
	let away_team_formation_map = new Map<
		string,
		Fixture_Player[]
	>();
	...
	async function check_live_fixtures(
		...
		// [🐞]
		const logs_name =
    LI_W_F_TAG + ' check_live_fixtures';
		const logs: string[] = [];
		...
		if (live_fixtures_map.has(fixture_id)) {
			...
			FIXTURE_LINEUPS.home.formation =
				live_fixtures_map.get(
					fixture_id
				)?.formations?.localteam_formation;
			...
			const FIREBASE_LINEUPS_DATA =
				live_fixtures_map.get(fixture_id)?.lineup
					?.data;
			const FIREBASE_BENCH_DATA =
				live_fixtures_map.get(fixture_id)?.bench
					?.data;
			...
			if (
				...
				const lineup_ids =
					FIREBASE_LINEUPS_DATA.map(
						(p) => p?.player_id
					);
				const bench_ids = FIREBASE_BENCH_DATA.map(
					(p) => p?.player_id
				);
				football_player_ids = [
					...new Set(
						football_player_ids.concat(
							lineup_ids.concat(bench_ids)
						)
					)
				];
				const player_data =
					await get_target_player_data(
						football_player_ids
					);
				const players_map =
					await generate_players_map(player_data);

				// [ℹ] home-team lineup [init]
				const home_team_lineup: Fixture_Player[] =
					FIREBASE_LINEUPS_DATA.length == 0
						? []
						: FIREBASE_LINEUPS_DATA.filter(
								(player) =>
				...
				FIXTURE_LINEUPS.away.lineup =
					away_team_lineup;
				FIXTURE_LINEUPS.home.bench =
					home_team_bench;
				FIXTURE_LINEUPS.away.bench =
					away_team_bench;
			}
			...
			for (const player of FIXTURE_LINEUPS.home
				...
				for (const fixture_lineup of FIREBASE_LINEUPS_DATA) {
					if (
						player?.player_id ==
						fixture_lineup?.player_id
					) {
						player.rating =
							fixture_lineup?.stats?.rating;
					}
				}
				...
				for (const live_event of FIXTURE_LINEUPS.events) {
					...
					if (
						player.player_id ==
						live_event.related_player_id
					) {
						if (
							live_event.type == 'substitution'
						) {
			...
			for (const player of FIXTURE_LINEUPS.home
				...
				for (const fixture_bench of FIREBASE_BENCH_DATA) {
					if (
						player?.player_id ==
						fixture_bench?.player_id
					) {
						player.rating =
							fixture_bench?.stats?.rating;
					}
				}
				// [ℹ] reset player events
				player.events = {
	...
	$: if (
		...
		let home_team_formation_arr_temp =
			FIXTURE_LINEUPS?.home?.formation.split('-');
		...
		for (const form_pos of home_team_formation_arr_temp) {
			...
			let form_pos_code =
				form_pos + count_pos_diff.toString();
			for (
				let i = rt_home_count;
				i < rt_home_count + form_pos_num;
				i++
			) {
				const player =
					FIXTURE_LINEUPS?.home?.lineup[i];
				if (
					home_team_formation_map.has(
						form_pos_code
					)
				) {
		...
		rt_home_count = 0;
		count_pos_diff = 0;
		let away_team_formation_arr_temp =
			FIXTURE_LINEUPS?.away?.formation.split('-');
		...
		let null_formation =
			FIXTURE_LINEUPS?.away?.lineup.filter(
				({ formation_position }) =>
				...
				: false;
		if (!null_formation) {
			FIXTURE_LINEUPS?.away?.lineup.sort(
				(a, b) =>
					parseFloat(
	...
	async function get_target_player_data(
		...
		BETARENA_HASURA_SURGICAL_JSONB_scores_football_players[]
	> {
		playerIdsArr = playerIdsArr.filter(function (
			el
		) {
			return el != undefined;
		});
