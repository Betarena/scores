...
<script lang="ts">
	...
	$: team_name =
		TEAM_DATA.team_name == null ||
		TEAM_DATA.team_name == undefined
			? '-'
			: TEAM_DATA.team_name;
	$: position =
		TEAM_DATA.position == null ||
		TEAM_DATA.position == undefined
			? '-'
			: TEAM_DATA.position;
	$: points =
		TEAM_DATA.points == null ||
		TEAM_DATA.points == undefined
			? '-'
			: TEAM_DATA.points;
	$: games_played =
		TEAM_DATA.games_played == null ||
		TEAM_DATA.games_played == undefined
			? '-'
			: TEAM_DATA.games_played;
	$: gavg =
		TEAM_DATA.gavg == null ||
		TEAM_DATA.gavg == undefined
			? '-'
			: parseFloat(
					TEAM_DATA.gavg.toString()
			  ).toFixed(2);
	$: cavg =
		TEAM_DATA.cavg == null ||
		TEAM_DATA.cavg == undefined
			? '-'
			: parseFloat(
					TEAM_DATA.cavg.toString()
			  ).toFixed(2);
	$: ycavg =
		TEAM_DATA.ycavg == null ||
		TEAM_DATA.ycavg == undefined
			? '-'
			: parseFloat(
					TEAM_DATA.ycavg.toString()
			  ).toFixed(2);
	$: ov15 =
		TEAM_DATA.ov15 == null ||
		TEAM_DATA.ov15 == undefined
			? '-'
			: parseFloat(
					TEAM_DATA.ov15.toString()
			  ).toFixed(0) + '%';
	$: ov25 =
		TEAM_DATA.ov25 == null ||
		TEAM_DATA.ov25 == undefined
			? '-'
			: parseFloat(
					TEAM_DATA.ov25.toString()
			  ).toFixed(0) + '%';
	$: winP =
		TEAM_DATA.winP == null ||
		TEAM_DATA.winP == undefined
			? '-'
			: parseFloat(
					TEAM_DATA.winP.toString()
			  ).toFixed(0) + '%';
        ...
        -->
				<div class="team-pos">
					<p
						...
						class:border-pos={TEAM_DATA.color_code ===
							'transparent'}
					>
						{position}
					</p>
...
{:else if TABLEMOBILEVIEW != undefined}
	<tr
		class="league-table-team-row"
		class:dark-background-1={$userBetarenaSettings.theme ==
			'Dark'}
