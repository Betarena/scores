...
//#endregion ➤ Assets Imports
  ...
  // ~~~~~~~~~~~~~~~~~~~~~
    ...
    dlog(`${FIX_W_T_TAG} widget_init()`, FIX_W_T_TOG, FIX_W_T_STY);
		...
		const response_all: Cache_Single_SportbookDetails_Data_Response[] =
			await get(
    ...
    dlog(`${FIX_W_T_TAG} select_fixtures_odds() cont.`, FIX_W_T_TOG, FIX_W_T_STY);
		...
		let target_season =
			FIXTURES_ODDS_DATA.seasons.find(
				({ season_id }) =>
					season_id ===
		...
		if (optView === 'round') {
			...
			for (
				let i = 0;
				i < target_season.rounds.length;
				i++
			) {
				const s_date = new Date(
					target_season.rounds[i].s_date
				);
				const e_date = new Date(
					target_season.rounds[i].e_date
				);
				const past_e_date =
					i == 0
						? null
						: new Date(
								target_season.rounds[i - 1].s_date
						  );
      ...
      dlog(`${FIX_W_T_TAG} week_end: ${week_end} | week_start: ${week_start}`, FIX_W_T_TOG, FIX_W_T_STY);
		...
		// [ℹ] identify "week" start/end dates
			...
			for (
				let i = 0;
				i < target_season.weeks.length;
				i++
			) {
				const s_date = new Date(
					target_season.weeks[i].s_date
				);
				const e_date = new Date(
					target_season.weeks[i].e_date
				);
				const past_e_date =
					i == 0
						? null
						: new Date(
								target_season.weeks[i - 1].s_date
						  );
				// [ℹ] case for LEAGUE on-going,
				// [ℹ] with user date matching an on-going fixtures-week
				if (
      ...
      dlog(`${FIX_W_T_TAG} week_end: ${week_end} | week_start: ${week_start}`, FIX_W_T_TOG, FIX_W_T_STY);
		 ...
		 */
		const fixtures_group_by_date = new Map<
			string,
		...
		for (const fixture of temp_fixtures_odds_arr) {
			...
			if (fixtures_group_by_date.has(fixDate)) {
				fixtures_group_by_date
					...
					.push(fixture);
				let fix_arr =
					fixtures_group_by_date.get(fixDate);
				fix_arr.sort(
					(a, b) =>
		...
		for (const [
			group_date,
			fixtures_arr
		] of fixtures_group_by_date.entries()) {
			const fixObj = {
				date: group_date,
    ...
    dlog(`${FIX_W_T_TAG} carusel_fixture_odds_data()`, FIX_W_T_TOG, FIX_W_T_STY);
		...
		if (optView === 'round') {
			const target_round =
				target_season.rounds.find(
					({ value }) => value == opt_view
				);
      ...
      dlog(`${FIX_W_T_TAG} week_end: ${week_end} | week_start: ${week_start}`, FIX_W_T_TOG, FIX_W_T_STY);
		...
		// [ℹ] identify "week" start/end dates
			...
			const target_week =
				target_season.weeks.find(
					({ name }) => parseInt(name) == opt_view
				);
...
<div id="widget-outer">
  ...
  -->
	...
	<div id="seo-widget-box">
        ...
        -->
			{#each FIXTURES_ODDS_DATA?.seasons[0].fixtures as item}
				{#if item?.fixture_link && item?.fixture_link[server_side_language]}
					<a
						href={item?.fixture_link[
							server_side_language
						]}
					>
						{item?.teams?.home?.name} vs. {item
							?.teams?.away?.name}
					</a>
				{/if}
				{#if item?.tip_link && item?.tip_link[server_side_language]}
					<a
						href={item?.tip_link[
							server_side_language
						]}
			...
			<div>
				<p
					...
					class:color-white={$userBetarenaSettings.theme ==
						'Dark'}
				>
					{FIXTURES_ODDS_T.no_info}
				</p>
				<p class="s-14 color-grey w-400">
					{FIXTURES_ODDS_T.no_info_desc}
				</p>
			</div>
		</div>
	{/if}
  ...
  [ℹ] -> error handling
  -->
	{#if browser && !noWidgetData && !refresh && $userBetarenaSettings.country_bookmaker && ready && showWidget}
		<!-- 
            ...
            -->
                ...
                -->
                    ...
                    -->
                      ...
                      -->
											{#if fixture?.tip_link && fixture?.tip_link[server_side_language]}
												<a
													rel="nofollow"
													aria-label="tip_link_redirect"
													href={fixture?.tip_link[
														server_side_language
													]}
													target="_blank"
													style="width: inherit;"
                          ...
                          -->
                            ...
                            "
														class:color-grey={fixture
															...
															fixture?.status !=
																'LIVE'}
														class:color-red-bright={fixture?.status ===
															'LIVE'}
													>
														{fixture?.teams?.home
															?.score}
                          ...
                          -->
													...
													>
														...
														>
															...
															>
																{fixture?.live_odds?.home?.value?.toFixed(
																	2
																)}
															</p>
															<img
													...
													>
														...
														>
															<p
																class="s-12 color-black-2 w-500"
															>
																{fixture?.live_odds?.draw?.value?.toFixed(
																	2
																)}
															</p>
															<img
													...
													>
														...
														>
															<p
																class="s-12 color-black-2 w-500"
															>
																{fixture?.live_odds?.away?.value?.toFixed(
																	2
																)}
															</p>
															<img
...
<style>
  ...
  */
	...
	div#dropdown-seasons
		div#dropdown-list-main-container
		div#dropdown-list-inner-container {
		max-height: 308px;
		overflow-y: scroll;
		overflow-x: hidden;
	}
	...
	/* width */
	div#dropdown-seasons
		div#dropdown-list-inner-container::-webkit-scrollbar {
		width: 4px;
	}
	/* track */
	div#dropdown-seasons
		div#dropdown-list-inner-container::-webkit-scrollbar-track {
		background: #f2f2f2;
		border-radius: 12px;
		margin: 8px;
	}
	/* handle */
	div#dropdown-seasons
		div#dropdown-list-inner-container::-webkit-scrollbar-thumb {
		background: #cccccc;
		border-radius: 12px;
	}
  ...
  ==================== */
	...
	.dark-background-1
		div#fix-odds-view-box
		div.fix-odds-view-opt-box:hover
		p {
