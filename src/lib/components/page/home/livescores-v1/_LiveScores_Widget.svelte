...
<script lang="ts">
	...
	async function listenBettingIcon(): Promise<{
		[key: string]: LiveScoreBookIcon;
	}> {
		if (requestedIcons) {
			bookIcons = bookIcons;
	...
	async function listenRealTimeGamesChange(
		table: string,
		all: boolean
	): Promise<LiveScoreLeague[]> {
		...
		onValue(
			ref(db_real, fullTable),
			(snapshot) => {
				data = [];

				let games: LiveScoreGame[] =
					snapshot.val();
				...
				for (var g in games) {
					...
					const newGame: LiveScoreLeagueGame = {
						...
						visitorScore:
							games[g].visitorteam_score,
						minute: games[g].minute,
						medias: games[g].medias,
						status: games[g].status,
						links: games[g].links,
						homeCards: games[g].cards
							? games[g].cards.filter(
									(x) =>
										x.team_name ==
										games[g].localteam
							  ).length
							: 0,
						visitorCards: games[g].cards
							? games[g].cards.filter(
									(x) =>
										x.team_name ==
										games[g].visitorteam
						...
						tips: games[g].tips,
						starting_at_ts:
							games[g].starting_at_ts,
						hour: games[g].starting_at
					};

					var l = data.filter(
						(x) => x.id === games[g].league_id
					)[0];
					if (l == null) {
						const league: LiveScoreLeague = {
							...
							flag: games[g].flag
								.toUpperCase()
								.replace('FLAGS', 'images/flags')
								.replace('.SVG', '.svg'),
							name: games[g].league,
							order: getLeagueOrders(
								games[g].league_id
							)
						};
						data.push(league);
				...
				TABLE_GAMES[base + snapshot.ref.key] =
					data;
				if (
					currentTable == snapshot.ref.key &&
					(!isMoreGames ||
						(isMoreGames && base.includes('all')))
				) {
	...
	function getTerm(t: any, i: number) {
		return LIVESCORES_TRANSLATIONS[
			server_side_language
		]
			? LIVESCORES_TRANSLATIONS[
					server_side_language
			  ].terms[t][i]
			: '';
	}
	...
	listenRealTimeGamesChange(
		'livescores_now',
		false
	);

	//Returns the order of all leagues
	function getLeagueOrders(league_id: number): {
		[key: string]: number;
	} {
		let retVal: { [key: string]: number } = {};
		for (var country in leagueSort) {
			retVal[country] = getLeagueOrder(
				league_id,
				country
			);
		}
	...
	function getLeagueOrder(
		...
		for (var v in leagueSort[country]) {
			if (
				leagueSort[country][v].league_id ==
				league_id
			)
	...
	async function widgetInit(): Promise<void> {
		...
		let translations: LiveScores_Football_Translation[] =
		...
		if ($userBetarenaSettings.geoJs)
			userGeo =
				$userBetarenaSettings.geoJs.country_code.toLowerCase();
	...
	function getTournamentLink(
		d: LiveScoreLeague,
		server_side_language: string
	): string {
		return LIVESCORES_FOOTBALL_TOURNAMENTS[d.id]
...
{#if !refresh}
	...
	{:then}
		{#if LIVESCORES_TRANSLATIONS[server_side_language]}
			...
			<div
				class="game-table-container {$userBetarenaSettings.theme ==
				'Dark'
					? 'theme-dark'
					: ''}"
			>
				...
				{#await listenRealTimeGamesChange(currentTable, false) then data}
					<div class="betarena-menu" style="">
						<span
							class="lnkAll {currentSelection == 0
								? 'lnkSelected'
								: ''}"
							on:click={() => {
								currentSelection = 0;
							...
							>{LIVESCORES_TRANSLATIONS[
								...
								: ''} ({currentTable !=
							'livescores_now'
								? CURRENT_TABLE.reduce(
										(a, b) => a + b.games.length,
										0
								  )
								: TABLE_GAMES[
										'livescores_table' + lastTable
								  ].reduce(
										(a, b) => a + b.games.length,
										0
								  )})</span
						>
						<span
							class="lnkLive {currentSelection ==
							1
								? 'lnkSelected'
								: ''}"
							on:click={() => {
								...
								CURRENT_TABLE =
									TABLE_GAMES[
										'livescores_tablelivescores_now'
									];
							}}
							>{LIVESCORES_TRANSLATIONS[
								server_side_language
										...
										{#each d.games.sort((x, y) => x.starting_at_ts - y.starting_at_ts) as g}
											...
											>
												...
												>
													...
													<div
														class="team second {g.localScore >
														g.visitorScore
															? 'loser'
															: ''} {g.localScore ==
														g.visitorScore
												...
												<div class="game-result">
													<div
														class="result first {g.localScore <
														g.visitorScore
															? 'loser'
															: ''} {g.localScore ==
														g.visitorScore
													...
													<div
														class="result second {g.localScore >
														g.visitorScore
															? 'loser'
															: ''} {g.localScore ==
														g.visitorScore
...
<style>
	...
	.theme-dark.game-table-container
	...
	.game-table-container .betarena-menu span,
	.game-table-container .betarena-menu span {
		width: 50%;
		font-weight: 500;
		font-size: 14px;
	...
	.in-progress .game-result .game-time,
	.in-progress .game-result .result {
		font-weight: 500 !important;
		font-size: 14px !important;
		text-align: center !important;
		color: #ff0a0a !important;
...
.elementor-shortcode{
	...
	.theme-dark .in-progress .game-time,
	.theme-dark .in-progress .result {
