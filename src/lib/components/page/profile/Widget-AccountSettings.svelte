...
<script lang="ts">
  ...
  async function upload_profile_picture (event): Promise < void > {
    ...
    uploadString(storageRef, profile_pic, 'data_url')
    .then(async(snapshot) => {
      dlog('🟢 Uploaded file!')
  ...
  async function remove_picture (): Promise < void > {
    ...
    deleteObject(desertRef).then(() => {
      dlog('ℹ️ removed from firebase - storage')
    }).catch((error) => {
      errlog(error)
    });
...
{#if modal_delete_show}
  <ModalDeleteAccount 
    on:toggle_delete_modal={() => modal_delete_show = false}
...
{#if modal_wallet_show}
  <ModalConnectWallet 
    on:toggle_delete_modal={() => modal_wallet_show = false}
    on:connect_wallet_action={() => connect_wallet()}
  />
{/if}
...
<ModalProfilePictureCrop 
  ...
  {modal_pic_crop_show}
  on:toggle_delete_modal={() => modal_pic_crop_show = false}
