...
import type {
  BetarenaUser,
  IScoreUser,
} from "$lib/types/types.user-settings.js";
 ...
 * @description
 *  📝 Final authentication step. Updates (1) `stores`, (2) `localStorage` and (3) `UI`.
 * @example
 *  [1]──────────────────────────────────────────────────────────────────
 *  │ successAuthComplete
 *  │ (
...
export async function successAuthComplete(
  ...
  return await tryCatchAsync(async (): Promise<boolean> => {
    // [🐞]
    log_v3({
    ...
    const /**
       ...
       */
      betarenaUser = await getFirestoreBetarenaUser(
        firebaseUser?.uid!,
        firebaseUser!,
        web3WalletAddress!,
        authProviderType!,
    ...
    userBetarenaSettings.updateData([["user-object", userObject]]);
 ...
 * @return { Promise < void > }
 */
export async function authWithMoralis(): Promise<void> {
  // ╭─────
  // │ NOTE: IMPORTANT |:| Check for browser environment.
  // ╰─────
  if (!browser) return;
...
async function getFirestoreBetarenaUser(
  ...
  return await tryCatchAsync(async (): Promise<BetarenaUser> => {
    // [🐞]
    log_v3({
    ...
    const /**
       ...
       */
      documentSnapshot = await getDoc(
        doc(db_firestore, "betarena_users", uid!),
      );
    // ╭─────
    // │ CHECK:
    ...
    const /**
       ...
       */
      dataRes0 = await new Betarena_User_Class().createUser({
        query: {
        ...
        body: {
          ...
          referredBy: getCookie(document.cookie)
    ...
    log_v3({
      strGroupName: "🚏 checkpoint ➤ getFirestoreBetarenaUser(..) // END",
      msgs: [`🔹 [var] dataRes0 :: ${JSON.stringify(dataRes0, null, 4)}`],
    });

    if (dataRes0.error) throw new Error(JSON.stringify(dataRes0.error));
    const /**
       * @description
       * 📝 Target user document snapshot.
       */
      dataRes1 = await getDoc(doc(db_firestore, "betarena_users", uid!));
