 ...
 * @return { Promise < void > }
 */
export async function selectLanguage(lang: string | NullUndef): Promise<void> {
  if (sessionStore.extract("lang") == lang || !lang) return;

  userBetarenaSettings.updateData([["lang", lang]]);
  ...
  dlogv2(
    ...
    [
      `🔹 [var] ➤ $userBetarenaSettings.lang: ${userBetarenaSettings.extract(
        "lang-user"
      )}`,
  ...
  // ╰─────
  document.documentElement.setAttribute("lang", lang == "br" ? "pt-BR" : lang);
  ...
  if (!checkNull(page?.error)) {
    const targetUrl: string = lang == "en" ? "/" : `/${lang}`;
    // [🐞]
    dlogv2(
  ...
  switch (page?.route.id) {
    ...
    case routeIdPageProfileAuthorCreate: {
      const pastLangV2: string = pastLang == "/" ? "/en" : pastLang,
        ...
        newURL: string =
          tempUrl.replace(`${pastLangV2}/`, `/${lang}`) + page.url.search;
      // [🐞]
      dlogv2(
    ...
    case routeIdSportstack:
      //[🐞]
      dlogv2("🚏 checkpoint ➤ selectLanguage(..) [x2]", [], true);
  ...
  // ╰─────
    ...
    // │ > replace path-name accordingly for "<lang>" - first occurance.
    // ╰─────
    newURL = newURL ?? page?.url.pathname.replace(pastLang, `/${lang}`);
  // ╭─────
  // │ NOTE:
...
export async function promiseValidUrlCheck(
  ...
  if (opts.competitionMainUrl)
    queryStr += `&competitionMainUrl=${opts.competitionMainUrl}`;
  if (opts.competitionUrl) queryStr += `&competitionUrl=${opts.competitionUrl}`;
  if (opts.authorArticleUrl)
    queryStr += `?authorArticleUrl=/${opts.authorArticleUrl}`;
  ...
  dlogv2(
    ...
    [`🔹 [var] ➤ if_M_0 :|: ${if_M_0}`, `🔹 [var] ➤ queryStr :|: ${queryStr}`],
    true
  );
  ...
  if (!if_M_0)
    return {
      isValid: false,
      objRedirect: { isRedirect: false, strRedirectUrl: null },
    };
  const /**
     * @description
     */
    response: any = await get(
 ...
 * @returns { void }
 */
export function preloadInvelidDataDebug(data: unknown[], urls: string[]): void {
  tryCatchAsync((): void => {
    const /**
       * @description
      ...
      indexesOf: (
        ...
        // eslint-disable-next-line no-unused-vars
        item: unknown
      ) => number[] = (arr: any[], item: unknown) => {
        return arr.reduce((accumulator, currentVal, currentIndex) => {
          return (
            currentVal === item && accumulator.push(currentIndex), accumulator
          );
        }, []);
      },
      /**
       ...
       */
      nullList: number[] = indexesOf(data, null);
    if (nullList.length == 0)
      // [🐞]
      dlog("🚏 checkpoint ➤ preloadInvelidDataDebug 🟩", true);
    // ╭─────
    ...
    if (nullList.length > 0) {
      ...
      for (const i of nullList) console.log(`\t🚩 ${urls[i]}`);
    }
...
export function preloadExitLogic(
  ...
  // [🐞]
  dlog(`${pageTag} ${((t1 - t0) / 1000).toFixed(2)} sec`, true);
 ...
 * @returns { void }
 */
export function preloadRedirect(redirectToUrl: string): void {
...
export async function promiseUrlsPreload(
  ...
  const /**
     ...
     * 📝 Data response.
     */
    dataRes0: any[] = await Promise.all(
      endpoints.map(async (_url: string): Promise<any> => {
        const // [🐞]
        ...
        // ╰─────
        if (resJson?.success) resJson = resJson.success.data;
        // [🐞]
        dlogv2(
  ...
  // [🐞]
  preloadInvelidDataDebug(dataRes0, endpoints);
