...
const CQ_Fixture_Incidents = new Bull (
  'CQ_Fixture_Incidents', 
  { 
    redis: { 
...
const cacheQueueProcessName = "CQ_Fixture_Incidents"
const cacheTarget = "REDIS CACHE | fixture incidents (all)"
const cache_data_addr = "fixture_incidents_data"
const cache_trans_addr = "fixture_incidents_trans"
// [ℹ] debug info
let logs = []
let t0;
let t1;
...
export async function POST(): Promise < unknown > {
  ...
  // [ℹ] otherwise prod.
    ...
    // [ℹ] producers [JOBS]
    const job = await CQ_Fixture_Incidents.add({});
...
CQ_Fixture_Incidents.process (async function (job, done) {
