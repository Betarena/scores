...
export async function GET(
	...
	if (geoPos) {
		...
		const response_usergeo = await get_target_hset_cache_data(league_list_cache_data_addr, geoPos);
		if (response_usergeo) {
			return json(response_usergeo);
		}

		// [ℹ] otherwise, return the "EN" version - default;
		const response_en = await get_target_hset_cache_data(league_list_cache_data_addr, 'en');
		if (response_en) {
	...
	if (lang) {
		const response_cache = await get_target_hset_cache_data(league_list_cache_trans_addr, lang);
		if (response_cache) {
