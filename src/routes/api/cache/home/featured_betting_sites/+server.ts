...
import {
  featured_bet_site_cache_data_addr,
  featured_bet_site_cache_trans_addr, get_target_hset_cache_data
} from '../../std_main';
...
export async function GET(req): Promise<unknown> {
	...
	if (geoPos) {
		...
		const response_en = await get_target_hset_cache_data(featured_bet_site_cache_data_addr, 'en');
		if (response_en) {
