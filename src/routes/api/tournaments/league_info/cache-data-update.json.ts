...
async function cacheTournamentsPageLeagueInfoData (url: string, json_cache: Hasura_League_Info_Widget_Data_Response) {
  try {
...
async function sportbookDetailsGeneration () {
  ...
  const finalCacheObj: Cache_Single_SportbookDetails_Data_Response = {
    geoPos: undefined,
  ...
  for (const geoSportbook of response.sportsbook_details_dev) {
    ...
    for (const [key, value] of Object.entries(geoSportbook.data)) {
      ...
      if (geoSportbook.data[key].position.toString() === '1') {
        finalCacheObj.beting_cta_link = value.register_link === undefined ? '' : value.register_link
        finalCacheObj.betting_site_logo = value.image === undefined ? '' : value.image
      }
    }
