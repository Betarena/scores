...
async function getFeaturedMatchData() {
	...
	const promise = await get_TargetFixtureOddsAndInfo(
		selectedFixture.widget_featured_match_selection[0]
	);
...
async function assignValueBetsData(): Promise<void> {
	...
	const sportbook_details: any = await getTargetGeoSportBookDetails(userGeo, siteName);
