...
async function main_trans_and_seo (
  ...
  */
  ...
  const objectFixOdds = response.scores_fixtures_h2h_translations
    .find(({ lang }) => lang === LANG)
...
async function get_target_fixture (
	...
	// [ℹ] obtain target historic_fixtures [fixture_id]
	const queryName = 'REDIS_CACHE_FIXTURE_HEAD_2_HEAD_0';
...
async function get_widget_translations (
  ...
  const queryName = "REDIS_CACHE_FIXTURE_HEAD_2_HEAD_2";
  const t0 = performance.now();
  const VARIABLES = {
		lang
	};
