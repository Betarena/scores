...
async function main(
	...
	const lineup_ids = fixture_data?.lineup_j.map(
		(p) => p?.player_id
	);
	const bench_ids = fixture_data?.bench_j.map(
		(p) => p?.player_id
	);
	// const subs_ids = [
	...
	const players_map = await generate_players_map(
		player_data
	);
	...
	const home_team_coach_name =
		fixture_data?.home_coach_j?.fullname || null;
	const home_team_coach_avatar =
		fixture_data?.home_coach_j?.image_path ||
		null;
		...
		fixture_data?.substitutions_j.length == 0
			...
			: fixture_data?.substitutions_j
					...
					.map((p) => ({
						...
						player_avatar_in:
							players_map.get(p.player_in_id)
								?.image_path_j || null,
						player_avatar_out:
							players_map.get(p.player_out_id)
								?.image_path_j || null
					}));
	// [ℹ] away-team
	...
	const away_team_coach_name =
		fixture_data?.away_coach_j?.fullname || null;
	const away_team_coach_avatar =
		fixture_data?.away_coach_j?.image_path ||
		null;
	...
	const home_team_obj: Team_Lineup = {
		...
		team_short_code:
			...
			home_team_name?.slice(0, 3).toUpperCase() ||
			null,
		team_rating:
			fixture_data?.teams_rating?.home_team ||
			null,
	...
	const away_team_obj: Team_Lineup = {
		...
		team_short_code:
			...
			away_team_name?.slice(0, 3).toUpperCase() ||
			null,
		team_rating:
			fixture_data?.teams_rating?.away_team ||
			null,
