...
async function main(
	...
	const response =
		await getTargetSeasonPlayersInfo([SEASON_ID]);
	...
	if (
		...
		console.log(
			`Target season ${SEASON_ID} current seasons to update`
		);
		console.log(`Exiting`);
		return;
	}
	...
	const [teamIdsArr, playerIdsArr] =
		await getTeamsAndPlayersIds(response);

	// [ℹ] [GET] target teams / players
	const response3 =
		await getTargetTeamsAndPlayers(
			teamIdsArr,
			playerIdsArr
		);

	// [ℹ] JSON-ARRAY to HASHMAP
	const [players_map, teams_map] =
		await generateTeamsAndPlayersMap(response3);
	...
	const season_top_player_obj: Tournament_Season_Top_Player =
		{};
	season_top_player_obj.season_id =
		season_data?.id;
	...
	if (
		season_data.squad !== null &&
		season_data.squad.length != 0
	) {
		for (const season_team of season_data.squad) {
			for (const season_team_player of season_team
				.squad.data) {
				// [ℹ] ignore those players with "NaN" rating
				if (
					season_team_player?.rating == null ||
					season_team_player?.rating == undefined
				) {
					continue;
				}
				...
				const target_player: BETARENA_HASURA_scores_football_players =
					players_map.get(
						season_team_player.player_id
					);
				const target_team: BETARENA_HASURA_scores_football_teams =
					teams_map.get(season_team.id); // [ℹ] unecessary [?];
				...
				top_player.rating = parseFloat(
					season_team_player.rating.toString()
				);
				...
				if (
					top_player?.player_name == null ||
					top_player?.position == null
				) {
					continue;
				}

				season_top_player_obj.top_players_rating.push(
					top_player
				);
			}

			season_top_player_obj.top_players_rating.sort(
				(a, b) =>
					parseFloat(b.rating.toString()) -
					parseFloat(a.rating.toString())
			);
			season_top_player_obj.top_players_rating.splice(
				50
			);
		}
	}
	...
	if (
		...
		for (const season_goalscorer of season_data.goalscorers) {
			...
			if (
				season_goalscorer?.goals == null ||
				season_goalscorer?.goals == undefined
			) {
				continue;
			}
			...
			const target_player: BETARENA_HASURA_scores_football_players =
				players_map.get(
					season_goalscorer.player_id
				);
			const target_team: BETARENA_HASURA_scores_football_teams =
				teams_map.get(season_goalscorer.team_id); // [ℹ] unecessary [?];
			...
			top_player.goals = season_goalscorer.goals;
			...
			season_top_player_obj.top_players_goals.push(
				top_player
			);
		}

		season_top_player_obj.top_players_goals.sort(
			(a, b) =>
				parseFloat(b.goals.toString()) -
				parseFloat(a.goals.toString())
		);
		season_top_player_obj.top_players_goals.splice(
			50
		);
	}

	if (
		season_data.assistscorers !== null &&
		...
		for (const season_assistscorer of season_data.assistscorers) {
			...
			if (
				season_assistscorer?.assists == null ||
				season_assistscorer?.assists == undefined
			) {
				continue;
			}
			...
			const target_player: BETARENA_HASURA_scores_football_players =
				players_map.get(
					season_assistscorer.player_id
				);
			const target_team: BETARENA_HASURA_scores_football_teams =
				teams_map.get(
					season_assistscorer.team_id
				); // [ℹ] unecessary [?];
			...
			season_top_player_obj.top_players_assists.push(
				top_player
			);
		}

		season_top_player_obj.top_players_assists.sort(
			(a, b) =>
				parseFloat(b.assists.toString()) -
				parseFloat(a.assists.toString())
		);
		season_top_player_obj.top_players_assists.splice(
			50
		);
	}
	...
	if (
		...
		for (const season_team of season_data.squad) {
			for (const season_team_player of season_team
				...
				if (
					season_team_player?.shots
						?.shots_total == null ||
					season_team_player?.shots
						?.shots_total == undefined
				) {
					continue;
				}
				...
				season_top_player_obj.top_players_total_shots.push(
					top_player
				);
			}

			season_top_player_obj.top_players_total_shots.sort(
				(a, b) =>
					parseFloat(b.total_shots.toString()) -
					parseFloat(a.total_shots.toString())
			);
			season_top_player_obj.top_players_total_shots.splice(
				50
			);
		}
	}
	...
	if (dev) {
		const data = JSON.stringify(
			season_top_player_obj,
			null,
			4
		);
		fs.writeFile(
			'./datalog/hasura-season_top_player_obj.json',
			data,
			(err) => {
				if (err) {
					console.error(err);
...
> {
	...
	while (true) {
		...
		const response: BETARENA_HASURA_top_players_season_details_query =
    ...
    */
		...
		season_details_arr =
			season_details_arr.concat(
				response.scores_football_seasons_details
			);
		...
		if (offset >= total_limit) {
			...
			logs.push(
				` ↳ total seasons: ${season_details_arr.length}`
			);
			logs.push(
				` ↳ exiting loop after ${counter} iterations`
			);
			break;
		}
...
async function getTeamsAndPlayersIds(
	...
	playerIdsArr = playerIdsArr.filter(
		(element) => {
	...
	logs.push(
		` ↳ num. of playerIdsArr: ${playerIdsArr.length}`
	);
	logs.push(
		` ↳ num. of teamIdsArr: ${teamIdsArr.length}`
	);

	return [teamIdsArr, playerIdsArr];
}
...
async function getTargetTeamsAndPlayers(
	...
	const queryName =
		'REDIS_CACHE_TOP_PLAYERS_ST_DATA_3';
	logs.push(`${queryName}`);
	const response: BETARENA_HASURA_top_players_query =
		await initGrapQLClient().request(
			REDIS_CACHE_TOP_PLAYERS_ST_DATA_3,
			VARIABLES
		);
...
> {
	...
	const t1 = performance.now();
	logs.push(
		` ↳ players_map generated with size: ${players_map.size}`
	);
	logs.push(
		` ↳ teams_map generated with size: ${teams_map.size}`
	);
	logs.push(
		` ↳ hashmap conversion: ${
			(t1 - t0) / 1000
