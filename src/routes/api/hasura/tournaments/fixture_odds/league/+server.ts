...
export async function GET(req): Promise<unknown> {
	...
	const leagueId: string =
		req.url['searchParams'].get('leagueId');
...
async function main(
	...
	const target_current_season_arr =
		await get_league_current_seasons(LEAGUE_ID);
	// [ℹ] exit
	if (
		target_current_season_arr == undefined ||
	...
	const SEASON_ID =
		target_current_season_arr[0].id;
