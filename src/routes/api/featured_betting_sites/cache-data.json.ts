...
export async function post({ params, request }, res): Promise < any > {
    ...
    const response_usergeo = await getFeaturedBettingSiteGeoPosFromCache(userGeo)
    // ... DEBUGGING;
    ...
    const response_en = await getFeaturedBettingSiteGeoPosFromCache('en')
    // ... DEBUGGING;
...
async function getFeaturedBettingSiteGeoPosFromCache(geoPos: string): Promise < FixtureResponse | Record < string, never > > {
    // ... TRY;
    try {
        // ... cached data retrival;
        const cached: string = await redis.hget('featured_betting_sites', geoPos);
        // ... check for `cached` data
        if (cached) {
            // ... convert the data from `string` to `JSON`
            const parsed: FixtureResponse = JSON.parse(cached);
            // ... DEBUGGING;
            if (dev) console.info(`Found featured_betting_sites ${geoPos} in cache`);
            // ... return, cached data;
            return parsed;
        }
        return
