...
export async function load({
	...
	const response_tournaments_seo: Cache_Single_Tournaments_SEO_Translation_Response =
		await fetch(
	...
	const response_tournaments_page_info: Cache_Single_Tournaments_Data_Page_Translation_Response =
		await fetch(
	...
	response_tournaments_seo.main_data = JSON.parse(
		JSON.stringify(
	...
	response_tournaments_seo.opengraph = JSON.parse(
		JSON.stringify(
	...
	const enItemAlt =
		response_tournaments_page_info.alternate_data.find(
			({ lang }) => lang === urlLang
		);
	...
	const response_league_info: Cache_Single_Tournaments_League_Info_Data_Response =
		await fetch(
	...
	const league_id =
		response_tournaments_page_info.data
			.tournament_id;
	...
	const response_top_players_translations: REDIS_CACHE_SINGLE_tournaments_top_player_widget_t_data_response =
		await fetch(
	...
	const response_top_players_data: REDIS_CACHE_SINGLE_tournaments_top_player_widget_data_response =
		await fetch(
	...
	const response_fixtures_odds_data: REDIS_CACHE_SINGLE_tournaments_fixtures_odds_widget_data_response =
		await fetch(
	...
	if (dev) {
		...
		if (response_tournaments_seo == undefined)
			console.log('response_tournaments_seo');
		if (
			response_tournaments_page_info == undefined
		)
			console.log(
				'response_tournaments_page_info'
			);
		if (response_league_info == undefined)
			console.log('response_league_info');
		if (
		...
		if (response_standings_data == undefined)
			console.log('response_standings_data');
		if (
		...
		if (response_top_players_data == undefined)
			console.log('response_top_players_data');
		if (
		...
		if (response_fixtures_odds_data == undefined)
			console.log('response_fixtures_odds_data');
	}
	...
	if (
		...
		return {
			...
			TOURNAMENT_DATA_TRANSLATED_COPIES:
				response_tournaments_page_info.alternate_data,
			TOURNAMENT_DATA:
				response_tournaments_page_info.data,
	...
	throw error(
		500,
		`Uh-oh! There has been an /{tournaments} page preloading error`
	);
}
