...
<script lang="ts" context="module">
    ...
    */
    for await (const item of response_seo_page.scores_seo_tournaments_dev) {
      // [ℹ] replace;
      item.main_data = JSON.parse(JSON.stringify(item.main_data).replaceAll("{lang}", 'en'));
      ...
      item.twitter_card = JSON.parse(JSON.stringify(item.twitter_card).replaceAll("{lang}", 'en'));
      ...
      item.opengraph = JSON.parse(JSON.stringify(item.opengraph).replaceAll("{lang}", 'en'));
