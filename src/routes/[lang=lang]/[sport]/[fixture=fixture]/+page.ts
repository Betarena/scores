...
import { dlog, F_DEBUG_STYLE, F_DEBUG_TAG, F_DEBUG_TOGGLE } from '$lib/utils/debug';

/** @type {import('./$types').PageLoad} */
export async function load({ url, params, fetch }): PageLoad {
  const t0 = performance.now();
	...
	const id =
		response_fixtures_page_info?.data?.id == undefined ? undefined : response_fixtures_page_info?.data?.id.toString();
	...
	if (response_scoreboard == undefined || use_hasura) {
		dlog(`${F_DEBUG_TAG} Non current_season fixture - loading from Hasura Directly`, F_DEBUG_TOGGLE, F_DEBUG_STYLE);
		response_scoreboard = await fetch(`/api/hasura/fixtures/scoreboard?fixture_id=${fixture_id}`, {
			method: 'GET'
		}).then((r) => r.json());
	}
	...
	if (response_lineups == undefined || use_hasura) {
		dlog(`${F_DEBUG_TAG} Non current_season fixture - loading from Hasura Directly`, F_DEBUG_TOGGLE, F_DEBUG_STYLE);
		response_lineups = await fetch(`/api/hasura/fixtures/lineups?fixture_id=${fixture_id}`, {
			method: 'GET'
		}).then((r) => r.json());
	}
	...
	if (response_incidents == undefined || use_hasura) {
		dlog(`${F_DEBUG_TAG} Non current_season fixture - loading from Hasura Directly`, F_DEBUG_TOGGLE, F_DEBUG_STYLE);
		response_incidents = await fetch(`/api/hasura/fixtures/incidents?fixture_id=${fixture_id}`, {
			method: 'GET'
		}).then((r) => r.json());
	}
	...
	const response_featured_betting_sites_translation: Cache_Single_Lang_Featured_Betting_Site_Translation_Response =
		await fetch(`/api/cache/home/featured_betting_sites?lang=${urlLang}`, {
			method: 'GET'
		}).then((r) => r.json());
	...
	if (response_statistics == undefined || use_hasura) {
		dlog(`${F_DEBUG_TAG} Non current_season fixture - loading from Hasura Directly`, F_DEBUG_TOGGLE, F_DEBUG_STYLE);
		response_statistics = await fetch(`/api/hasura/fixtures/statistics?fixture_id=${fixture_id}`, {
			method: 'GET'
		}).then((r) => r.json());
	}
	...
	if (response_content == undefined || use_hasura) {
		dlog(`${F_DEBUG_TAG} Non current_season fixture - loading from Hasura Directly`, F_DEBUG_TOGGLE, F_DEBUG_STYLE);
		response_content = await fetch(`/api/hasura/fixtures/content?fixture_id=${fixture_id}&lang=${urlLang}`, {
			method: 'GET'
		}).then((r) => r.json());
	}
	...
	if (response_about == undefined || use_hasura) {
		dlog(`${F_DEBUG_TAG} Non current_season fixture - loading from Hasura Directly`, F_DEBUG_TOGGLE, F_DEBUG_STYLE);
		response_about = await fetch(`/api/hasura/fixtures/about?fixture_id=${fixture_id}&lang=${urlLang}`, {
			method: 'GET'
		}).then((r) => r.json());
	}
	...
	const response_fixtures_odds_translations: REDIS_CACHE_SINGLE_tournaments_fixtures_odds_widget_t_data_response =
		await fetch(`/api/cache/tournaments/fixtures_odds?lang=${urlLang}`, {
			method: 'GET'
		}).then((r) => r.json());
	...
	if (response_h2h == undefined || use_hasura) {
		dlog(`${F_DEBUG_TAG} Non current_season fixture - loading from Hasura Directly`, F_DEBUG_TOGGLE, F_DEBUG_STYLE);
		response_h2h = await fetch(`/api/hasura/fixtures/head-2-head?fixture_id=${fixture_id}`, {
			method: 'GET'
		}).then((r) => r.json());
	}
	...
	if (dev) {
		...
		if (response_featured_betting_sites_translation == undefined)
			console.log('response_featured_betting_sites_translation is undefined');
		...
		if (response_fixtures_odds_translations == undefined)
			console.log('response_fixtures_odds_translations is undefined');
