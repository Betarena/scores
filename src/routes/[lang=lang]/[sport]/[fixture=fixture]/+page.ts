...
export async function load({
	...
	const response_fixtures_seo: REDIS_CACHE_SINGLE_fixtures_seo_response =
		await fetch(
	...
	const response_fixtures_page_info: REDIS_CACHE_SINGLE_fixtures_page_info_response =
		await fetch(
	...
	const id =
		response_fixtures_page_info?.data?.id ==
		undefined
			? undefined
			: response_fixtures_page_info?.data?.id.toString();
	const league_name =
		response_fixtures_page_info?.data
			?.league_name;
	const country_id =
		response_fixtures_page_info?.data?.country_id;
	...
	const fixture_day =
		response_fixtures_page_info?.data
			...
			? undefined
			: response_fixtures_page_info?.data?.fixture_day.replace(
					'T00:00:00',
					''
			  );
	const venue_name =
		response_fixtures_page_info?.data?.venue_name;
	const venue_city =
		response_fixtures_page_info?.data?.venue_city;

	const response_country_translation: REDIS_CACHE_SINGLE_general_countries_translation =
		await fetch(
	...
	const country =
		response_country_translation?.translations[
			urlLang
		];

	response_fixtures_page_info.data.country =
		country;
	response_fixtures_page_info.data.sport =
		'football';
	...
	response_fixtures_seo.main_data = JSON.parse(
		JSON.stringify(
			response_fixtures_seo.main_data
		)
			.replace(/{id}/g, id)
			.replace(/{lang}/g, lang)
			...
			.replace(
				/{data.venue.data.name}/g,
				venue_name
			)
			.replace(
				/{data.venue.data.city}/g,
				venue_city
			)
	);
	...
	response_fixtures_seo.twitter_card = JSON.parse(
		JSON.stringify(
			response_fixtures_seo.twitter_card
		)
			.replace(/{id}/g, id)
			.replace(/{lang}/g, lang)
			.replace(/{sport}/g, sport)
	...
	response_fixtures_seo.opengraph = JSON.parse(
		JSON.stringify(
			response_fixtures_seo.opengraph
		)
			.replace(/{id}/g, id)
			.replace(/{lang}/g, lang)
			.replace(/{sport}/g, sport)
	...
	const enItemAlt =
		response_fixtures_page_info.alternate_data[
			'en'
		];
	response_fixtures_seo.main_data.canonical =
		enItemAlt;
	...
	let response_scoreboard: REDIS_CACHE_SINGLE_scoreboard_data =
		await fetch(
	...
	if (
		response_scoreboard == undefined ||
		use_hasura
	) {
		if (dev)
			console.debug(
				'Non current_season fixture - loading from Hasura Directly'
			);
	...
	const response_scoreboard_translation: REDIS_CACHE_SINGLE_scoreboard_translation =
		await fetch(
	...
	// NOTE:IMPORTANT: can be null -load from hasura
	let response_lineups: REDIS_CACHE_SINGLE_lineups_data =
		await fetch(
	...
	const response_lineups_translation: REDIS_CACHE_SINGLE_lineups_translation =
		await fetch(
	...
	// NOTE:IMPORTANT: can be null -load from hasura
	let response_incidents: REDIS_CACHE_SINGLE_incidents_data =
		await fetch(
	...
	const response_incidents_translation: REDIS_CACHE_SINGLE_incidents_translation =
		await fetch(
	...
	const response_featured_betting_sites_translation: Cache_Single_Lang_Featured_Betting_Site_Translation_Response =
		await fetch(
	...
	// NOTE:IMPORTANT: can be null -load from hasura
	let response_statistics: REDIS_CACHE_SINGLE_statistics_data =
		await fetch(
	...
	const response_statistics_translation: REDIS_CACHE_SINGLE_statistics_translation =
		await fetch(
	...
	// NOTE:IMPORTANT: can be null -load from hasura
	let response_content: REDIS_CACHE_SINGLE_content_data[] =
		await fetch(
	...
	const response_content_translation: REDIS_CACHE_SINGLE_content_translation =
		await fetch(
	...
	// NOTE:IMPORTANT: can be null -load from hasura
	let response_about: REDIS_CACHE_SINGLE_about_data =
		await fetch(
	...
	const response_about_translation: REDIS_CACHE_SINGLE_about_translation =
		await fetch(
	...
	const response_votes_translation: REDIS_CACHE_SINGLE_votes_translation =
		await fetch(
	...
	const response_probability_translation: REDIS_CACHE_SINGLE_probabilities_translation =
		await fetch(
	...
	const response_fixtures_odds_translations: REDIS_CACHE_SINGLE_tournaments_fixtures_odds_widget_t_data_response =
		await fetch(
	...
	// NOTE:IMPORTANT: can be null -load from hasura
	let response_h2h: Fixture_Head_2_Head =
		await fetch(
			`/api/cache/fixtures/about?fixture_id=${fixture_id}&lang=${urlLang}`,
			{
				method: 'GET'
	...
	const response_h2h_translation: REDIS_CACHE_SINGLE_h2h_translation =
		await fetch(
	...
	// [🐞]
	const league_id =
		response_scoreboard?.league_id;
	if (dev) console.log('league_id', league_id);

	const response_standings_translations: REDIS_CACHE_SINGLE_tournament_standings_translation =
		await fetch(
	...
	const response_standings_data: REDIS_CACHE_SINGLE_tournament_standings_data =
		await fetch(
	...
	if (dev) {
		...
		if (response_fixtures_seo == undefined)
			console.log(
				'response_fixtures_seo is undefined'
			);
		if (response_fixtures_page_info == undefined)
			console.log(
				'response_fixtures_page_info is undefined'
			);
		if (response_scoreboard == undefined)
			console.log(
				'response_scoreboard is undefined'
			);
		if (
			response_scoreboard_translation == undefined
		)
			console.log(
				'response_scoreboard_translation is undefined'
			);
		if (response_lineups == undefined)
			console.log(
				'response_lineups is undefined'
			);
		if (response_lineups_translation == undefined)
			console.log(
				'response_lineups_translation is undefined'
			);
		if (response_incidents == undefined)
			console.log(
				'response_incidents is undefined'
			);
		if (
			response_incidents_translation == undefined
		)
			console.log(
				'response_incidents_translation is undefined'
			);
		if (
			...
			console.log(
				'response_featured_betting_sites_translation is undefined'
			);
		if (response_statistics == undefined)
			console.log(
				'response_statistics is undefined'
			);
		if (
			response_statistics_translation == undefined
		)
			console.log(
				'response_statistics_translation is undefined'
			);
		if (response_content == undefined)
			console.log(
				'response_content is undefined'
			);
		if (response_content_translation == undefined)
			console.log(
				'response_content_translation is undefined'
			);
		if (response_about_translation == undefined)
			console.log(
				'response_about_translation is undefined'
			);
		if (response_votes_translation == undefined)
			console.log(
				'response_votes_translation is undefined'
			);
		if (response_h2h == undefined)
			console.log('response_h2h is undefined');
		if (response_h2h_translation == undefined)
			console.log(
				'response_h2h_translation is undefined'
			);
		if (
			...
			console.log(
				'response_fixtures_odds_translations is undefined'
			);
		if (
			response_standings_translations == undefined
		)
			console.log(
				'response_standings_translations is undefined'
			);
		if (response_standings_data == undefined)
			console.log(
				'response_standings_data is undefined'
			);
	}
	...
	throw error(
		500,
		`Uh-oh! There has been an /{fixture} page preloading error`
	);
}
