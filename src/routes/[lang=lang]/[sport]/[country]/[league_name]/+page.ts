...
export async function load({
  ...
  const response_tournaments_seo: Cache_Single_Tournaments_SEO_Translation_Response = await fetch(`/api/cache/_main_/pages_and_seo?lang=` + urlLang + "&page=tournaments", {
    method: 'GET'
  }).then((r) => r.json());

  const response_tournaments_page_info: Cache_Single_Tournaments_Data_Page_Translation_Response = await fetch(`/api/cache/_main_/pages_and_seo?url=` + url.pathname + "&page=tournaments", {
    method: 'GET'
  }).then((r) => r.json());
  ...
  const response_league_info: Cache_Single_Tournaments_League_Info_Data_Response = await fetch(`/api/cache/tournaments/league_info?url=` + url.pathname, {
    method: 'GET'
  }).then((r) => r.json());

  const league_id = response_tournaments_page_info.data.tournament_id;

  const response_standings_translations: Cache_Single_Tournaments_League_Standings_Translation_Data_Response = await fetch(`/api/cache/tournaments/standings?lang=` + urlLang, {
    method: 'GET'
  }).then((r) => r.json());

  // [ℹ] standings-widget cache data is dependent on the LEAGUE-ID;
  const response_standings_data: Cache_Single_Tournaments_League_Standings_Info_Data_Response = await fetch(`/api/cache/tournaments/standings?league_id=` + league_id, {
    method: 'GET'
  }).then((r) => r.json());

  const response_top_players_translations: REDIS_CACHE_SINGLE_tournaments_top_player_widget_t_data_response = await fetch(`/api/cache/tournaments/top_players?lang=` + urlLang, {
    method: 'GET'
  }).then((r) => r.json());

  // [ℹ] top-players-widget cache data is dependent on the LEAGUE-ID;
  const response_top_players_data: REDIS_CACHE_SINGLE_tournaments_top_player_widget_data_response = await fetch(`/api/cache/tournaments/top_players?league_id=` + league_id, {
    method: 'GET'
  }).then((r) => r.json());

  // [ℹ] fixtures_odds
  const response_fixtures_odds_translations: REDIS_CACHE_SINGLE_tournaments_fixtures_odds_widget_t_data_response = await fetch(`/api/cache/tournaments/fixtures_odds?lang=` + urlLang, {
    method: 'GET'
  }).then((r) => r.json());

  // [ℹ] fixtures-odds-widget cache data is dependent on the LEAGUE-ID;
  const response_fixtures_odds_data: REDIS_CACHE_SINGLE_tournaments_fixtures_odds_widget_data_response = await fetch(`/api/cache/tournaments/fixtures_odds?league_id=` + league_id, {
    method: 'GET'
  }).then((r) => r.json());
